...
% mesh generation
n_en   = 3;               % number of nodes in a triangle
n_el_x = 60;              % number of elements in x-dir
n_el_y = 60;              % number of elements in y-dir
n_el   = 2 * n_el_x * n_el_y; % total number of triangular elements (each quad is divided into 2 triangles)
...
for ex = 1 : n_el_x
  for ey = 1 : n_el_y
    IEN(el_count, 1) = (ey-1) * n_np_x + ex;               
    IEN(el_count, 2) = (ey-1) * n_np_x + ex + 1;           
    IEN(el_count, 3) = ey * n_np_x + ex + 1;                
    el_count = el_count + 1;

    IEN(el_count, 1) = (ey-1) * n_np_x + ex;               
    IEN(el_count, 3) = ey * n_np_x + ex + 1;                
    IEN(el_count, 2) = ey * n_np_x + ex;                    
    el_count = el_count + 1;
  end
end
...
for ii = 1 : n_np
  ...
  if index > 0
    ...
    % modify disp with the boundary data
  end
end
